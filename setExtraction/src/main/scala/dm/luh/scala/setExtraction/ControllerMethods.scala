...
/*Method that organizes the pre processing steps  */
object ControllerMethods {   
  //Preprocessing methods calls
  def firstSetPreprocessingSteps(tweetIn:String) :String =
  { 
    //Expected format
    //(label, tweet-id, timestamp, query, user, tweet)    
    var tweetSplit:Array[String]  =  Utils.tweetToArray(tweetIn, "\",\"")    
    var sentimentGroundThruth = ""
    var tweet = ""    
    try {
      sentimentGroundThruth =  tweetSplit(0).replaceAll("[\" ]", "") //cluster
      tweet = tweetSplit(5).replaceAll("[\"]", "") //cluster
    } catch {
      case t: Throwable => t.printStackTrace()
    }    
    /*
    var sentimentGroundThruth = sentimentGroundThruth  match {
      case "neutral"   => 2
      case "positive"  => 4
      case "negative"  => 0
      case _  => 5
    ...
    return "\""+sentimentGroundThruth.toString()+"\",\""+processedTweet+"\",\""+tweet+"\",\""+ listHashTags +"\",\""+ listSlangs1+"\",\""+listSlangs2+"\""    
  }
  
  //method for checking tweets in other languages 
  def secondSetPreprocessingSteps(tweetIn:String) :String = 
  {    
    var tweetSplit:Array[String]  =  Utils.tweetToArray(tweetIn,"\",\"")
  ...
  def thirdSetPreprocessingSteps (rddTweets:RDD[String], sc:SparkContext) : RDD[String] = 
  {     
  ...
  def fourthSetPreprocessingSteps(tweet:String) :String = 
  {
    //(0 -> sentiment, 1-> preprocessTweet, 2 -> Orig Tweet,  3->Hash; 4->Slangs1, 5->Slangs2, 6 ->ListNoFreq)
  ...
  def setActualYesPredYes() = 
  {
    actYesPredYes = 0     
  } 
  
  def setActualNoPredNeutral() = 
  {
    actNoPredNeu = 0
  }
  
  def setActualNoPredNoSent() =
  {
    actNoPredNoSent = 0     
  } 
  
  def setActualYesPredNeutral() =
  {
    actYesPredNeu = 0      
  }
  
  def setActualYesPredNoSent() =
  {
    actYesPredNoSent = 0      
  }
  
  def setActualNoOthers() = 
  {
    ActualNoOthers = 0 
  }
  
  def setActualYesOthers() =
  {
    actYesOth = 0
